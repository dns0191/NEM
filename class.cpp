...
void MultiGroupNode::updateTransverseLeakage(int u, int g)
{
	...
	Node* l_node = getNeighborNode(u, LEFT_SIDE);
	Node* r_node = getNeighborNode(u, RIGHT_SIDE);
    ...
    const double L_l = (DL0_l / h_l + DL0_c / h_c) / (beta_l + beta_c);
    const double L_r = (DL0_c / h_c + DL0_r / h_r) / (beta_c + beta_r);
...
Node* MultiGroupNode::getNeighborNode(int direction, bool side)
{
    return 0;
}
